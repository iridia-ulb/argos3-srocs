...
namespace argos {
   ...
   void CSRoCSQTOpenGLUserFunctions::DrawInWorld() {
      CSpace::TMapPerType& mapBuilderBots = m_pcSpace->GetEntitiesByType("builderbot");
      for(const std::pair<const std::string, CAny>& c_entity : mapBuilderBots) {
         CBuilderBotEntity& cBuilderBot = *any_cast<CBuilderBotEntity*>(c_entity.second);
         //DrawPoint(cBuilderBot.GetEmbodiedEntity().GetOriginAnchor().Position, CColor::BLUE, 500.0f);
      }
   }
   ...
   void CSRoCSQTOpenGLUserFunctions::Draw(CBuilderBotEntity& c_builderbot) {
      std::cerr << c_builderbot.GetId() << "?" << std::endl;
